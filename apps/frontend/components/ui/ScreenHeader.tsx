...
export function ScreenHeader({ title, subtitle, onBack, icon, action }: Props) {
  ...
  return <View style={[styles.wrap,{backgroundColor:colors.surface,borderBottomColor:colors.border},shadows.soft]}>
    <TouchableOpacity accessibilityRole="button" accessibilityLabel={tx("Go back")} onPress={onBack} style={[styles.back,{backgroundColor:colors.backgroundAlt}]}><MaterialCommunityIcons name="arrow-left" size={21} color={colors.text}/></TouchableOpacity>
    ...
    <View style={styles.copy}><Text style={[styles.title,{color:colors.text}]}>{title}</Text>{subtitle?<Text style={[styles.subtitle,{color:colors.textMuted}]}>{subtitle}</Text>:null}</View>{action}
  </View>;
}
