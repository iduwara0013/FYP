...
export function TradeDealScreen({ profile, initialConversation, onBack }: Props) {
  ...
  const send = async () => {
    ...
    } catch (error) { setMessage(text); Alert.alert("Message not sent", error instanceof Error ? error.message : "Please try again."); }
    finally { setSending(false); }
  };

  const saveTerms = async () => {
    if (Number(quantity) <= 0 || Number(price) <= 0 || !deliveryMethod.trim() || !deliveryDate.trim()) {
      return Alert.alert("Incomplete deal", "Enter quantity, price, delivery method and delivery date.");
    }
    setSavingTerms(true);
    try {
      ...
      Alert.alert(updated.status === "agreed" ? "Trade agreed" : "Terms proposed", updated.status === "agreed" ? "The listing is now closed and saved under Agreed trades." : `${otherName} must confirm these exact terms before the listing closes.`);
    } catch (error) { Alert.alert("Could not save terms", error instanceof Error ? error.message : "Please try again."); }
    finally { setSavingTerms(false); }
  };

  const cancel = () => Alert.alert(profile.role === "buyer" ? "Not interested in these goods?" : "End this discussion?", "The harvest will be unlocked and shown to all buyers in the marketplace again.", [
    { text: "Continue discussion", style: "cancel" },
    { text: "Release goods", style: "destructive", onPress: async () => { try {
      ...
      onBack();
    } catch (error) { Alert.alert("Could not unlock", error instanceof Error ? error.message : "Please try again."); } } },
  ]);

  return <SafeAreaView edges={["top"]} style={[styles.safe, { backgroundColor: colors.background }]}>
    <KeyboardAvoidingView style={styles.safe} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <ScreenHeader title={otherName || "Private trade"} subtitle={`${conversation.crop} · Private discussion`} icon="chat-processing-outline" onBack={onBack} />
      <View style={[styles.lockBanner, { backgroundColor: conversation.status === "agreed" ? colors.primarySoft : colors.infoSoft }]}>
        ...
        <Text style={{ flex: 1, color: conversation.status === "agreed" ? colors.primary : colors.info, fontSize: 12, fontWeight: "800" }}>{conversation.status === "agreed" ? "Trade agreed · removed from the open marketplace" : "Locked · only this farmer and buyer can access the discussion"}</Text>
      </View>
      <ScrollView ref={scrollRef} style={styles.chat} contentContainerStyle={styles.chatContent} onContentSizeChange={() => scrollRef.current?.scrollToEnd({ animated: false })}>
        <View style={[styles.goodsCard, { backgroundColor: colors.surface, borderColor: colors.border }]}><View style={[styles.goodsIcon, { backgroundColor: colors.primarySoft }]}><MaterialCommunityIcons name="sprout" size={25} color={colors.primary} /></View><View style={{ flex: 1 }}><Text style={[styles.goodsTitle, { color: colors.text }]}>{conversation.listingTitle || conversation.crop}</Text><Text style={[styles.goodsMeta, { color: colors.textMuted }]}>{conversation.listedQuantity} kg · Rs {conversation.listedPrice}/kg · {conversation.location}</Text></View></View>
        {messages.length ? messages.map((item) => { const mine = item.senderId === userId; return <View key={item.id} style={[styles.messageRow, mine && styles.mineRow]}><View style={[styles.bubble, { backgroundColor: mine ? colors.primary : colors.surface, borderColor: colors.border }]}><Text style={{ color: mine ? colors.primaryContrast : colors.text, fontSize: 14, lineHeight: 20 }}>{item.text}</Text><Text style={{ color: mine ? colors.primaryContrast : colors.textMuted, opacity: .72, fontSize: 9, marginTop: 4, textAlign: "right" }}>{new Date(item.sentAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</Text></View></View>; }) : <View style={styles.start}><MaterialCommunityIcons name="shield-lock-outline" size={32} color={colors.textMuted} /><Text style={{ color: colors.textMuted, textAlign: "center", lineHeight: 19 }}>Start discussing quality, available quantity, price and delivery. This conversation is private.</Text></View>}
        {conversation.status === "awaiting_confirmation" ? <View style={[styles.proposal, { backgroundColor: colors.warningSoft, borderColor: colors.warning }]}><Text style={[styles.proposalTitle, { color: colors.text }]}>Final terms proposed</Text><Text style={{ color: colors.textSecondary, lineHeight: 20 }}>{conversation.finalQuantity} kg × Rs {conversation.finalPrice}/kg{"\n"}{conversation.deliveryMethod} · {conversation.deliveryDate}{"\n"}Total: Rs {conversation.totalAmount?.toLocaleString()}</Text><Text style={{ color: colors.warning, fontWeight: "800", fontSize: 11, marginTop: 7 }}>{conversation.proposedBy === userId ? `Waiting for ${otherName} to confirm` : "Review and confirm the exact terms below"}</Text></View> : null}
        {conversation.status === "agreed" ? <><View style={[styles.agreed, { backgroundColor: colors.primarySoft }]}><MaterialCommunityIcons name="check-decagram" size={42} color={colors.primary} /><Text style={[styles.agreedTitle, { color: colors.primary }]}>Deal finalized</Text><Text style={[styles.agreedText, { color: colors.text }]}>{conversation.finalQuantity} kg at Rs {conversation.finalPrice}/kg{"\n"}Total Rs {conversation.totalAmount?.toLocaleString()}{"\n"}{conversation.deliveryMethod} · {conversation.deliveryDate}</Text></View><Text style={[styles.contactHeading, { color: colors.text }]}>Contact details</Text><ContactCard role="Farmer" contact={conversation.farmerContact} fallbackName={conversation.farmerName} colors={colors} /><ContactCard role="Buyer" contact={conversation.buyerContact} fallbackName={conversation.buyerName} colors={colors} /></> : null}
        {showTerms && !closed ? <View style={[styles.terms, { backgroundColor: colors.surface, borderColor: colors.border }]}><Text style={[styles.termsTitle, { color: colors.text }]}>Final deal details</Text><View style={styles.row}><Field value={quantity} setValue={setQuantity} placeholder="Quantity kg" numeric colors={colors} /><Field value={price} setValue={setPrice} placeholder="Rs per kg" numeric colors={colors} /></View><Field value={deliveryMethod} setValue={setDeliveryMethod} placeholder="Buyer pickup or farmer delivery" colors={colors} /><Field value={deliveryDate} setValue={setDeliveryDate} placeholder="Delivery date (YYYY-MM-DD)" colors={colors} /><Text style={[styles.total, { color: colors.text }]}>Total: Rs {((Number(quantity) || 0) * (Number(price) || 0)).toLocaleString()}</Text><TouchableOpacity disabled={savingTerms} onPress={saveTerms} style={[styles.confirm, { backgroundColor: colors.primary }]}>{savingTerms ? <ActivityIndicator color={colors.primaryContrast} /> : <Text style={{ color: colors.primaryContrast, fontWeight: "900" }}>{conversation.status === "awaiting_confirmation" && conversation.proposedBy !== userId ? "Confirm these terms" : "Propose final terms"}</Text>}</TouchableOpacity></View> : null}
      </ScrollView>
      {!closed ? <><TouchableOpacity onPress={cancel} style={[styles.releaseBar, { backgroundColor: colors.dangerSoft ?? "#FEE2E2" }]}><MaterialCommunityIcons name="lock-open-outline" size={18} color={colors.danger} /><Text style={{ color: colors.danger, fontSize: 12, fontWeight: "900" }}>{profile.role === "buyer" ? "Not interested · Release goods" : "End discussion · Make goods available again"}</Text></TouchableOpacity><View style={[styles.footer, { backgroundColor: colors.surface, borderColor: colors.border }]}>{!showTerms ? <TouchableOpacity onPress={() => setShowTerms(true)} style={[styles.termsButton, { backgroundColor: colors.primarySoft }]}><MaterialCommunityIcons name="file-sign" size={20} color={colors.primary} /><Text style={{ color: colors.primary, fontWeight: "900", fontSize: 11 }}>Finalize</Text></TouchableOpacity> : null}<TextInput value={message} onChangeText={setMessage} placeholder="Message..." placeholderTextColor={colors.textMuted} style={[styles.messageInput, { color: colors.text, backgroundColor: colors.background, borderColor: colors.border }]} /><TouchableOpacity onPress={send} disabled={sending || !message.trim()} style={[styles.send, { backgroundColor: colors.primary }]}><MaterialCommunityIcons name="send" size={20} color={colors.primaryContrast} /></TouchableOpacity></View></> : null}
