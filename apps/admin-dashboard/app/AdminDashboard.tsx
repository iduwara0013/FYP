...
export function AdminDashboard() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "Could not load SmartCrop services.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return <div className={dark ? "admin-app dark" : "admin-app"}>
    <aside className={menuOpen ? "sidebar open" : "sidebar"}>
      <div className="brand"><span className="brand-mark">S</span><span>SmartCrop<small>ADMIN CONSOLE</small></span></div>
...
function buildRecords(data: LiveData | null): Record<string, TableRecord> {
  ...
  return {
    ...
    "AI monitoring": { headers: ["Service", "Provider", "Mode", "Integration", "Status"], rows: [[data.agentHealth.service ?? "SmartCrop AI", data.agentHealth.llm?.provider ?? "—", data.agentHealth.demoMode ? "Demo" : "Production", neo4jStatus(data.agentHealth.neo4j), data.agentHealth.status ?? "Unavailable"]] },
